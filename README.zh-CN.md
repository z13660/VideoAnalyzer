...
![先后分析两段范围，中间空隙灰显](docs/analysing-a-range.png)

*在 60 秒素材上先后分析两段：QP 与运动行在 5–15 秒和 18–28 秒有数据，中间那段因为没有测过而灰显；
上下三条来自包表的行不受影响。*
...
* 范围分析会在末尾差一两帧：解码器被给定的时长截断，所以最后一帧的 QP 来自补洞而非 dump。
  色带外观正常，只有播放头正好停在那帧时右侧当前值略有偏差。
